	...
	Create the schema and table in your Supabase SQL Editor:

	```sql
	create schema find_the_bug;

	create table find_the_bug.analyses (
	...
	alter table find_the_bug.analyses enable row level security;

	create policy "users can only see their own analyses"
	  on find_the_bug.analyses for select
	  using (auth.uid() = user_id);

	create policy "users can only insert their own analyses"
	  on find_the_bug.analyses for insert
